...
const albums = ["all", "dessert", "restaurant", "dinner"];
function Gallery() {
    ...
    <section ref={galleryRef} className="text-gray-600 body-font">
      <div className="container py-24 px-0 mx-0">
          ...
          {albums.map((album, index) => {
            ...
            return (
              <React.Fragment key={index}>
                <button
                  tabIndex={index + 1}
                  className={`px-3 focus:outline-none uppercase cursor-pointer text-${BtnTxtClr}`}
                  onFocus={handleAlbumClick}>
                  {album}
                </button>
        ...
        <div className="flex flex-wrap -my-4 w-screen">
          {albumState === "all"
            ? gallery.map((image) => <ImageCard key={image.id} image={image} />)
            : gallery
                .filter((image) =>
                  image.album.toLowerCase().includes(albumState)
                )
                .map((filteredImage) => (
                  <ImageCard key={filteredImage.id} image={filteredImage} />
                ))}
        </div>
      </div>
    </section>
